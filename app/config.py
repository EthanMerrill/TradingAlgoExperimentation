...
class Config:
    ...
    def load_environment_variables(self):
        ...
        if self.ENVIRONMENT not in ['dev', 'qa', 'prod']:
            print(
                f"Warning: Invalid ENVIRONMENT value '{self.ENVIRONMENT}'. Must be 'dev', 'qa', or 'prod'. Defaulting to 'dev'.")
        ...
        if missing_vars:
            print(
                f"Warning: Missing required environment variables for {self.ENVIRONMENT} environment: {missing_vars}")
    ...
    def load_json_config(self):
        ...
        try:
            ...
            self.MAX_NEW_POSITIONS_PER_DAY = trading.get(
                'max_new_positions', 2)
            ...
            print(
                f"Warning: Configuration file {config_file} not found. Using default values.")
            self.setup_trading_parameters()
            self.setup_backtesting_parameters()
        except json.JSONDecodeError as e:
            print(
                f"Error: Invalid JSON in {config_file}: {e}. Using default values.")
            self.setup_trading_parameters()
            self.setup_backtesting_parameters()
