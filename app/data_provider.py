...
class DataProvider:
    ...
    Alpaca clients are created lazily on first access so that importing
    (or instantiating) a DataProvider has no network-client side effects —
    useful for tests and for config that changes at runtime.
    """
    ...
    def _ensure_clients(self) -> None:
        """Create the Alpaca clients on first use (idempotent)."""
        ...
        if not alpaca_config['api_key'] or not alpaca_config['secret_key']:
            logger.warning(
                "No Alpaca API credentials found. Data provider will have "
    ...
    @property
    def historical_client(self) -> Optional[StockHistoricalDataClient]:
        """Alpaca historical-data client (created on first access)."""
        self._ensure_clients()
        return self._historical_client

    @property
    def trading_client(self) -> Optional[TradingClient]:
        """Alpaca trading client (created on first access)."""
        self._ensure_clients()
