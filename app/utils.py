...
class TradingCalendar:
    ...
    def next_trading_day(self, date: Optional[datetime] = None) -> datetime:
    ...
    def previous_trading_day(self, date: Optional[datetime] = None) -> datetime:
    ...
    def check_correlation(returns1: pd.Series, returns2: pd.Series) -> float:
        ...
        try:
            ...
            if pd.isna(corr_value):
                return 0.0
            # Type guard to ensure we have a numeric value
            if isinstance(corr_value, (int, float, np.number)):
                return float(corr_value)
            else:
                return 0.0  # Non-numeric correlation, return 0
